Imports System.Web.UI.WebControls

Public Class Home

    Private Sub Guna2ImageCheckBox1_CheckedChanged(sender As Object, e As EventArgs) Handles Guna2ImageCheckBox1.CheckedChanged
        If Guna2ImageCheckBox1.Checked Then
            pnlSide.Width = 52
            Guna2HtmlLabel3.Visible = False
            pnlContent.Width = 905
            pnlContent.Location = New Point(79, pnlContent.Location.Y)

        Else
            pnlSide.Width = 195
            Guna2HtmlLabel3.Visible = True
            pnlContent.Width = 751
            pnlContent.Location = New Point(233, pnlContent.Location.Y)
        End If

    End Sub

    Private Sub Guna2CircleButton2_Click(sender As Object, e As EventArgs) Handles Guna2CircleButton2.Click
        Me.WindowState = FormWindowState.Minimized
    End Sub

    Private Sub Guna2ToggleSwitch1_CheckedChanged(sender As Object, e As EventArgs) Handles Guna2ToggleSwitch1.CheckedChanged
        If Guna2ToggleSwitch1.Checked Then
            Me.BackColor = Color.FromArgb(11, 36, 71)
            pnlSide.FillColor = Color.FromArgb(25, 55, 109)
            pnlSide.FillColor2 = Color.FromArgb(25, 55, 109)
            Guna2HtmlLabel3.ForeColor = Color.FromName("white")
            pnlContent.FillColor = Color.FromArgb(87, 108, 188)
            pnlContent.FillColor2 = Color.FromArgb(87, 108, 188)
        Else
            Me.BackColor = Color.FromKnownColor(KnownColor.Control)
            pnlSide.FillColor = Color.FromName("white")
            pnlSide.FillColor2 = Color.FromName("white")
            Guna2HtmlLabel3.ForeColor = Color.FromArgb(255, 96, 0)
            pnlContent.FillColor = Color.FromName("white")
            pnlContent.FillColor2 = Color.FromName("white")

        End If
    End Sub

    Private Sub Guna2CircleButton1_Click(sender As Object, e As EventArgs) Handles Guna2CircleButton1.Click
        End
    End Sub
End Class